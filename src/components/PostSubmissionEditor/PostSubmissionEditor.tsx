...
const PostSubmissionEditor: FC<Props> = ({
  defaultFeaturedImage = { sourceUrl: "", id: "" },
  defaultTitle = "",
  defaultContentHTML = "",
}) => {
  const [titleContent, setTitleContent] = React.useState(defaultTitle);
  const [contentHTML, setContentHTML] = React.useState(defaultContentHTML);
  const [featuredImage, setFeaturedImage] =
    React.useState<ImageState>(defaultFeaturedImage);
  ...
  //
  useEffect(() => {
    setTitleContent(defaultTitle);
  }, [defaultTitle]);

  useEffect(() => {
    setFeaturedImage(defaultFeaturedImage);
  }, [defaultFeaturedImage.id]);
  //
  ...
  return (
    <div className="nc-PostSubmissionEditor flex justify-center">
